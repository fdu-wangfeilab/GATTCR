...
class Mulgat_only_gat(nn.Module): 
    def __init__(self, tcr_num=300, feature_dim=768, attention_head_num=4, attention_hidden_size=128, num_gat_layers=2, drop_out=0.4):
        super(Mulgat_only_gat, self).__init__()
        self.tcr_num = tcr_num
        self.feature_dim = feature_dim
        self.num_gat_layers = num_gat_layers
        ...
        self.encoder1 = nn.Sequential(nn.Linear(attention_hidden_size, attention_hidden_size), nn.LeakyReLU(), nn.Dropout(p=drop_out))
        self.encoder2 = nn.Sequential(nn.Linear(attention_hidden_size, 2))

        self.dropout = nn.Dropout(p=drop_out)
        self.softmax = nn.Softmax(dim=1)

    def forward(self, x, adj, frequency_features=None, v_gene_features=None, caTCR_tag=False, return_features=False):
        x = self.dropout(x)
        ...
        combined_out = self.encoder1(x_max)
        combined_out = self.encoder2(combined_out)
        combined_out = self.softmax(combined_out)

        if return_features:
            return combined_out, top_indices, x_max
    ...
    def validate_model(model, valid_sps, valid_lbs, valid_mat, frequency_features, v_gene_features, device, valid_features=None):
        ...
        if valid_features is not None:
            ...
            tcrbert = TCR_Bert(model_path="/home/juhengwei/GAT_all/tcr-bert/tcr-bert-main/model_path/", src_dir="/home/juhengwei/GAT_all/tcr-bert/tcr-bert-main/tcr/")
            features = tcrbert.process_sps(valid_sps)
    ...
    def training(sps, lbs, adjs, tcr_num, lr, ep, dropout, log_inr, model_f, aa_f, device, v_gene_dict, shuffle=False, valid_sps=None, valid_lbs=None, valid_mat=None, valid_freq=None, valid_vgene=None):
        best_auc, best_f1 = 0, 0
        tcrbert = TCR_Bert(model_path="/home/juhengwei/GAT_all/tcr-bert/tcr-bert-main/model_path/", src_dir="/home/juhengwei/GAT_all/tcr-bert/tcr-bert-main/tcr/")
        features = tcrbert.process_sps(sps)
        valid_features = tcrbert.process_sps(valid_sps) if valid_sps is not None else None
        ...
        model = Mulgat_only_gat(tcr_num=tcr_num, feature_dim=768, drop_out=dropout, num_gat_layers=2).cuda()
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss().to(device)
        ...
        for epoch in range(ep):
            ...
            if valid_sps is not None and valid_lbs is not None and valid_mat is not None:
                valid_acc, valid_auc, valid_f1 = Mulgat_only_gat.validate_model(model, valid_sps, valid_lbs, valid_mat, None, None, device, valid_features=valid_features)
                print('Validation Accuracy:', valid_acc)
                print('Validation AUC:', valid_auc)
                print('Validation f1 score:', valid_f1)
                if valid_acc > max_acc:
    ...
    def prediction_with_umap(sps, dismat, model_f, tcr_num, device, true_labels):
        # 加载模型（只使用 GAT）
        model = Mulgat_only_gat(
            tcr_num=tcr_num, feature_dim=768, attention_head_num=4, 
            attention_hidden_size=128, drop_out=0.2, 
            num_gat_layers=2
        ).to(device)
...
class Mulgat_only_gat_gene(nn.Module):
    def __init__(self, tcr_num=300, feature_dim=768, attention_head_num=4, attention_hidden_size=128, num_gat_layers=2, drop_out=0.4, v_gene_vocab_size=100):
        super(Mulgat_only_gat_gene, self).__init__()
        self.tcr_num = tcr_num
        self.feature_dim = feature_dim
        self.num_gat_layers = num_gat_layers
        ...
        self.post_fusion_dropout = nn.Dropout(p=drop_out)
        fused_dim = (attention_hidden_size + 1) ** 2
        self.encoder1 = nn.Sequential(nn.Linear(fused_dim, attention_hidden_size), nn.LeakyReLU(), nn.Dropout(p=drop_out))
        self.encoder2 = nn.Sequential(nn.Linear(attention_hidden_size, 2))

        self.dropout = nn.Dropout(p=drop_out)
        self.softmax = nn.Softmax(dim=1)

    def forward(self, x, adj, v_gene_features, caTCR_tag=False, return_features=False):
        x = self.dropout(x)
        ...
        batch_size, tcr_num = v_gene_features.shape[:2]
        v_gene_features = self.v_gene_embedding(v_gene_features)
        v_gene_features = v_gene_features.view(batch_size * tcr_num, -1)
        v_gene_features = F.elu(self.fc_vgene1(v_gene_features))
        v_gene_features = self.vgene_dropout(v_gene_features)
        ...
        combined_out = self.encoder1(out)
        combined_out = self.encoder2(combined_out)
        combined_out = self.softmax(combined_out)

        if return_features:
            return combined_out, top_indices, out
    ...
    def validate_model(model, valid_sps, valid_lbs, valid_mat, v_gene_features, device, valid_features=None):
        model.eval()
        if valid_features is not None:
            features = valid_features
        else:
        ...
        with torch.no_grad():
            outputs = model(features_tensor, adjacencies_tensor, v_gene_features_tensor)
    ...
    def training(sps, lbs, adjs, tcr_num, lr, ep, dropout, log_inr, model_f, aa_f, device, v_gene_dict, shuffle=False, valid_sps=None, valid_lbs=None, valid_mat=None, valid_vgene=None):
        best_auc, best_f1 = 0, 0
        tcrbert = TCR_Bert(model_path="/home/juhengwei/GAT_all/tcr-bert/tcr-bert-main/model_path/", src_dir="/home/juhengwei/GAT_all/tcr-bert/tcr-bert-main/tcr/")
        features = tcrbert.process_sps(sps)
        ...
        v_gene_features = np.array([[Mulgat_only_gat_gene.v_gene_to_numeric(tcr[1], v_gene_dict) for tcr in sample] for sample in sps])
        ...
        model = Mulgat_only_gat_gene(tcr_num=tcr_num, feature_dim=768, drop_out=dropout, v_gene_vocab_size=len(v_gene_dict), num_gat_layers=2).cuda()
        ...
        dataset = TensorDataset(features_tensor, adjacencies_tensor, labels_tensor, v_gene_features_tensor)
        loader = DataLoader(dataset, batch_size=64, shuffle=shuffle)
        ...
        for epoch in range(ep):
            total_loss = 0
            for batch_x, batch_adj, batch_y, batch_vgene in loader:
                batch_x, batch_adj, batch_y, batch_vgene = batch_x.to(device), batch_adj.to(device), batch_y.to(device), batch_vgene.to(device)

                outputs = model(batch_x, batch_adj, batch_vgene)
                loss = criterion(outputs, batch_y)
                total_loss += loss.item()
            ...
            if valid_sps is not None and valid_lbs is not None and valid_mat is not None and valid_vgene is not None:
                valid_acc, valid_auc, valid_f1 = Mulgat_only_gat_gene.validate_model(model, valid_sps, valid_lbs, valid_mat, valid_vgene, device, valid_features=valid_features)
                print('Validation Accuracy:', valid_acc)
                print('Validation AUC:', valid_auc)
                print('Validation f1 score:', valid_f1)
...
class Mulgat_only_gat_freq_only(nn.Module):
    def __init__(self, tcr_num=300, feature_dim=768, attention_head_num=4,
                 attention_hidden_size=128, num_gat_layers=2, drop_out=0.4,
                 frequency_dim=1):
        super(Mulgat_only_gat_freq_only, self).__init__()
        self.tcr_num = tcr_num
        self.feature_dim = feature_dim
        self.num_gat_layers = num_gat_layers
        self.frequency_dim = frequency_dim
        ...
        self.gat_layers = nn.ModuleList([
            nn.ModuleList([
                GraphAttentionLayerV2(feature_dim if i == 0 else self.attention_hidden_size,
                                       self.attention_head_size, dropout=drop_out)
                for _ in range(attention_head_num)
            ]) for i in range(num_gat_layers)
        ])
        ...
        # 连接频率特征与GAT特征后的维度
        final_dim = attention_hidden_size + 128

        self.encoder1 = nn.Sequential(
            nn.Linear(final_dim, attention_hidden_size),
            nn.LeakyReLU(),
    ...
    def forward(self, x, adj, frequency_features, caTCR_tag=False, return_features=False):
        x = self.dropout(x)
        ...
        combined_out_pre = torch.cat((x_max, log_frequency_features), dim=-1)
        combined_out = self.encoder1(combined_out_pre)
        combined_out = self.encoder2(combined_out)
        combined_out = self.softmax(combined_out)

        if return_features:
            return combined_out, top_indices, combined_out_pre
    ...
    def validate_model(model, valid_sps, valid_lbs, valid_mat, frequency_features, device, valid_features=None):
        model.eval()
        if valid_features is not None:
            features = valid_features
        else:
    ...
    def training(sps, lbs, adjs, tcr_num, lr, ep, dropout, log_inr, model_f, aa_f, device, v_gene_dict, shuffle=False, valid_sps=None, valid_lbs=None, valid_mat=None, valid_freq=None):
        best_auc, best_f1 = 0, 0
        tcrbert = TCR_Bert(model_path="/home/juhengwei/GAT_all/tcr-bert/tcr-bert-main/model_path/", src_dir="/home/juhengwei/GAT_all/tcr-bert/tcr-bert-main/tcr/")
        features = tcrbert.process_sps(sps)
        if valid_sps is not None:
        ...
        model = Mulgat_only_gat_freq_only(tcr_num=tcr_num, feature_dim=768, drop_out=dropout, frequency_dim=1, num_gat_layers=2).cuda()
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss().to(device)
        ...
        dataset = TensorDataset(features_tensor, adjacencies_tensor, labels_tensor, frequency_features_tensor)
        loader = DataLoader(dataset, batch_size=64, shuffle=shuffle)
        ...
        for epoch in range(ep):
            ...
            if valid_sps is not None and valid_lbs is not None and valid_mat is not None and valid_freq is not None:
                valid_acc, valid_auc, valid_f1 = Mulgat_only_gat_freq_only.validate_model(model, valid_sps, valid_lbs, valid_mat, valid_freq, device, valid_features=valid_features)
                print('Validation Accuracy:', valid_acc)
                print('Validation AUC:', valid_auc)
                print('Validation f1 score:', valid_f1)
                if valid_acc > max_acc:
    ...
    def prediction_with_umap(sps, dismat, model_f, tcr_num, device, true_labels):
        model = Mulgat_only_gat_freq_only(
            tcr_num=tcr_num, feature_dim=768, attention_head_num=4,
            attention_hidden_size=128, drop_out=0.2,
            frequency_dim=1,
            num_gat_layers=2
        ...
        for i in range(0, len(features), batch_size):
            ...
            with torch.no_grad():
                outputs, top_indices, fused_features = model(
                    batch_features, batch_adjacencies,
                    batch_frequency_features,
                    caTCR_tag=True, return_features=True
                )

            batch_probs = outputs[:, 1].cpu().numpy()
            probs.extend(batch_probs)
            high_dim_features.append(fused_features.cpu().numpy())
