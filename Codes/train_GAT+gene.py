...
from network_test_fold import DeepLION2_mulgat_fre_vgene, Mulgat_fre_vgene_fusion, Mulgat_vgene_fusion_freq, Mulgat_only_gat_gene
...
def test_model(net, sps_dir, aa_f, model_f, tcr_num, device, filter_seq, record_r, mask_ratio):
    ...
    test_dist_mat_file = '/home/juhengwei/GAT_all/tcr-bert/GATTCR/test_dist_mat/THCA_test_dist_mat_300.npy'
    ...
    for fold_idx in range(5):
        ...
        probs = Mulgat_only_gat_gene.prediction_with_umap(
            test_sps, dist_mat_test, fold_model_path, tcr_num, device, v_gene_dict, test_labels)
...
def training_model(net, sps_dir, valid_sps_dir, tcr_num, lr, ep, dropout, log_inr, aa_f, model_f, device, loss, alpha,
    ...
    training_sps, training_labels, training_spnames, training_mask = read_samples(sps_dir, tcr_num, filter_seq,
                                                                                  mask_ratio)
    full_dist_mat = get_full_dist_mat(training_sps, tcr_num=tcr_num, file_path="/home/juhengwei/GAT_all/tcr-bert/GATTCR/dist_mat_data/GICA_train_full.npy")
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(kf.split(training_sps)):
        ...
        # 统一 v_gene_dict，计算 val 的 v_gene 编码
        val_vgene = np.array([[Mulgat_only_gat_gene.v_gene_to_numeric(tcr[1], v_gene_dict) for tcr in sample] for sample in val_sps])

        # 记录损失
        loss, acc, auc, f1 = Mulgat_only_gat_gene.training(
            sps=train_sps, 
            lbs=train_labels, 
            adjs=train_dist_mat, 
            tcr_num=tcr_num, 
