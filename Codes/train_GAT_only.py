...
from network_test_fold import Mulgat_only_gat
...
def test_model(net, sps_dir, aa_f, model_f, tcr_num, device, filter_seq, record_r, mask_ratio):
    ...
    for fold_idx in range(5):
        ...
        # 使用 GAT-only 模型进行预测（不含频率和基因特征）
        probs = Mulgat_only_gat.prediction_with_umap(
            test_sps, dist_mat_test, fold_model_path, 
            tcr_num=tcr_num, device=device, 
            true_labels=test_labels  # 不再使用 v_gene_dict
        )
...
def training_model(net, sps_dir, valid_sps_dir, tcr_num, lr, ep, dropout, log_inr, aa_f, model_f, device, loss, alpha,
    ...
    # 加载全距离矩阵（一次性，避免每折重复计算）
    full_dist_mat = get_full_dist_mat(training_sps, "/home/juhengwei/GAT_all/tcr-bert/GATTCR/dist_mat_data/CMV_train_full.npy")
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(kf.split(training_sps)):
        ...
        # 训练
        _, acc, auc, f1 = Mulgat_only_gat.training(
            sps=train_sps,
            lbs=train_labels,
            adjs=train_dist_mat,
            tcr_num=tcr_num,
    ...
    # 打印整体结果
    accs, aucs, f1s = zip(*fold_metrics)
