...
from network_test_fold import DeepLION2_mulgat_fre_vgene, Mulgat_fre_vgene_fusion, Mulgat_vgene_fusion_freq, Mulgat_only_gat_freq_only
...
def get_dist_mat(samples, file_path):
    if os.path.exists(file_path):
        ...
        with open(file_path, 'wb') as f:
            np.save(f, dist_mat)
        return dist_mat
def get_full_dist_mat(samples, file_path, force_recompute=False):
    if (not force_recompute) and os.path.exists(file_path):
        return np.load(file_path, allow_pickle=True)
    else:
        dist_mat = cal_distmat(samples, 300, 0, distance_function, cores=24)
...
def test_model(net, sps_dir, aa_f, model_f, tcr_num, device, filter_seq, record_r, mask_ratio):
    ...
    test_dist_mat_file = '/home/juhengwei/GAT_all/tcr-bert/GATTCR/test_dist_mat/CMV_test_dist_mat_300.npy'
    dist_mat_test = get_full_dist_mat(test_sps, test_dist_mat_file)
    ...
    for fold_idx in range(5):
        ...
        probs = Mulgat_only_gat_freq_only.prediction_with_umap(test_sps, dist_mat_test, fold_model_path, tcr_num, device, test_labels)
...
def training_model(net, sps_dir, valid_sps_dir, tcr_num, lr, ep, dropout, log_inr, aa_f, model_f, device, loss, alpha,
    ...
    training_sps, training_labels, training_spnames, training_mask = read_samples(sps_dir, tcr_num, filter_seq, mask_ratio)
    full_dist_mat = get_full_dist_mat(training_sps, tcr_num=tcr_num, file_path="/home/juhengwei/GAT_all/tcr-bert/GATTCR/dist_mat_data/CMV_train_full.npy")

    if data_balance:
        training_sps, training_labels = utils.data_balance(training_sps, training_labels)
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(kf.split(training_sps)):
        ...
        loss, acc, auc, f1 = Mulgat_only_gat_freq_only.training(
            sps=train_sps,
            lbs=train_labels,
            adjs=train_dist_mat,
            tcr_num=tcr_num,
