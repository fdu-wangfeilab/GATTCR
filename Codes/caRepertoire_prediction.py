...
def test_model(net, sps_dir, aa_f, model_f, tcr_num, device, filter_seq, record_r, mask_ratio):
    ...
    test_sps, test_labels, test_spnames, test_mask = read_samples_tyh(sps_dir, lables_dir="/mnt/sdb/tyh/new_MIL/final_files/append/2/test_labels.npy")
    # Make predictions.
    if net == "DeepLION":
        probs = DeepLION.prediction(test_sps, model_f, aa_f, tcr_num, device)
        with open("/home/juhengwei/GAT_all/tcr-bert/DeepLION2-main/Results/results_DeepLION_append_2_0512.tsv", 'w') as f:
            for prob in probs:
                f.write(f"{prob}\n")
    elif net == "TransMIL":
        probs = TransMIL.prediction(test_sps, model_f, aa_f, tcr_num, device, test_mask)
        with open("/home/juhengwei/GAT_all/tcr-bert/DeepLION2-main/Results/results_TransMIL_append_2_0512.tsv", 'w') as f:
            for prob in probs:
                f.write(f"{prob}\n")
    elif net == "BiFormer":
        probs = BiFormer.prediction(test_sps, model_f, aa_f, tcr_num, device)
        with open("/home/juhengwei/GAT_all/tcr-bert/DeepLION2-main/Results/results_BiFormer_append_2_0512.tsv", 'w') as f:
            for prob in probs:
                f.write(f"{prob}\n")
    elif net == "DeepLION2":
        probs = DeepLION2.prediction(test_sps, model_f, aa_f, tcr_num, device)
        with open("/home/juhengwei/GAT_all/tcr-bert/DeepLION2-main/Results/results_DeepLION2_append_2_0512.tsv", 'w') as f:
            for prob in probs:
                f.write(f"{prob}\n")
    elif net == "MINN_SA":
        probs = MINN_SA.prediction(test_sps, model_f, aa_f, tcr_num, device)
        with open("/home/juhengwei/GAT_all/tcr-bert/DeepLION2-main/Results/results_MINN_SA_append_2_0512.tsv", 'w') as f:
            for prob in probs:
                f.write(f"{prob}\n")
    else:
...
def training_model(net, sps_dir, valid_sps_dir, tcr_num, lr, ep, dropout, log_inr, aa_f, model_f, device, loss, alpha,
    ...
    # Get data.
    training_sps, training_labels, training_spnames, training_mask = read_samples_tyh(sps_dir, lables_dir="/mnt/sdb/tyh/new_MIL/final_files/append/2/train_labels.npy")
    ...
    valid_sps, valid_labels, valid_spnames, valid_mask = read_samples_tyh(["/mnt/sdb/tyh/new_MIL/final_files/append/2/val_file_name.npy"], lables_dir="/mnt/sdb/tyh/new_MIL/final_files/append/2/val_labels.npy")
    # Training model.
    if net == "DeepLION":
        if loss[0] == "W":
            # Pretraining.
